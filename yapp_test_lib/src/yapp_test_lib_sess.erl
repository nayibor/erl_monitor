...
kill_session(Arg,Cookie)->
		...
		case yaws_api:find_cookie_val(Cookie, H#headers.cookie) of
			[] ->
				error;
			Cookie_Unique ->
				yaws_api:delete_cookie_session(Cookie_Unique),
				ok
		end.
...
get_user_data(Arg,Cookie)->
		...
		case yaws_api:find_cookie_val(Cookie, H#headers.cookie) of
			...
			Cookie_Unique ->
				case yaws_api:cookieval_to_opaque(Cookie_Unique) of
					{ok, _OP=#session_data{id=_Id,fname=Name,userdata=_UseData,links_allowed=Links_Allowd}} ->						
						[{user_name,Name},{user_data,get_sidebar_links(Links_Allowd)}]; 
					{error, no_session} ->
						error
				end
		end.
...
get_sidebar_links(List_cat)->
    lists:foldl(fun({C,ListEl},Acc)-> [{C,lists:filter(fun({_Fi,_Si,_Ti,Fti})-> Fti =:= sidebar end,ListEl)} | Acc]   end ,[],List_cat).
