...
function Item(id,unit_price,stock_avail,name){
	...
	//value here has now been changed to hold the new stock .
	//this will now be used to calculate quant_sale,cost also	
    this.setQuant=function(quant_sale){
        ...
        if(quant_sale < 0 || quant_sale > this.stock_avail ){
            ...
            this.new_stock=parseInt(quant_sale,10);
            this.quant_sale=this.stock_avail-this.new_stock;
            this.cost=this.unit_price*this.quant_sale;
    ...
    this.setQuantRecv=function(quant_sale){
        ...
        if(quant_sale <= this.stock_avail || quant_sale < 1  ){
            ...
            this.new_stock=parseInt(quant_sale,10);
            this.quant_sale=this.new_stock-this.stock_avail;
            this.cost=this.unit_price*this.quant_sale;
...
var product={
    ...
    build_item:function(item){
      ...
      //  $(td_quant_sale).attr("min","0");
          $(td_quant_id).attr("class","item_for_sale quant_sale");
		  $(td_quant_id).html(item.quant_sale);
      //  $(td_quant_id).append(td_quant_sale);
        ...
        //this will hold the new td for the new stock which will also hold the new stock also
        var td_new_stock=document.createElement("td");
        var td_new_stock_input=document.createElement("input");
		$(td_new_stock_input).val(item.new_stock);        
        $(td_new_stock_input).attr("class","new_stock");
        $(td_new_stock_input).attr("type","number");
        $(td_new_stock_input).attr("step","1");
        $(td_new_stock_input).attr("min","0");
        $(td_new_stock_input).html(item.new_stock);
        $(td_new_stock).append(td_new_stock_input);
    ...
    edit_interface:function(item){
        ...
        $(tr_table).find(".quant_sale").html(item.getQuant());
    ...
    init:function(){
        ...
        $(".new_stock").live('change keyup',function(event){
         
            var itemId=$(this).closest("tr").data('id');
            var item= transaction.getItem(itemId);
            var old_quant=item.getNewStock();
