...
outa(Arg,'GET',["yapp_test","roles","search_roles"])->
		...
		case  yaws_api:queryvar(Arg,"filter") of
		   {ok,Filter} ->
				...
				{ok,UiData} = yapp_test_roles_search:render([{yapp_prepath,yapp:prepath(Arg)},{data,Roles}]),		
				{html,UiData}; 
			undefined ->
				Roles = yapp_test_lib_usermod:get_roles(),
				{ok,UiData} = yapp_test_roles_search:render([{yapp_prepath,yapp:prepath(Arg)},{data,Roles}]),		
...
outa(Arg,'POST',["yapp_test","roles","save_add_role"])->
		...
		case  yaws_api:postvar(Arg,"id") of
		   {ok,Edit_id_val} ->
		        ...
		        case yapp_test_lib_usermod:edit_role(list_to_integer(Edit_id_val),list_to_binary(Sname),list_to_binary(Lname)) of 
					ok ->
		   ...
		   undefined ->   
		        {ok,Sname} = yaws_api:postvar(Arg, "sname"),
		        {ok,Lname} = yaws_api:postvar(Arg, "lname"),
		        case yapp_test_lib_usermod:add_role(list_to_binary(Sname),list_to_binary(Lname)) of 
					ok ->
						yapp_test_lib_util:message_client(200,"Role added successfully");
					{error,Reason} ->
						yapp_test_lib_util:message_client(500,atom_to_list(Reason))
				end				
		end;
...
outa(_Arg,'GET',["yapp_test","roles","get_role_links",Roleid])->
		...
		{ok,UiData} = yapp_test_edit_roles_links:render([{links,Links},{rolelinks,Rlinks}]),
		{html,UiData};
...
outa(Arg,'POST',["yapp_test","roles","save_role_links"])->
		...
		case  yaws_api:postvar(Arg,"roleid") =:= {ok,[]} orelse
		      ...
		      yaws_api:postvar(Arg,"new_links") =:= undefined of
					true ->
					...
					false ->
						...
						case yapp_test_lib_usermod:add_role_link_list(Id,Links_save) of 
							{error,Reason} ->
