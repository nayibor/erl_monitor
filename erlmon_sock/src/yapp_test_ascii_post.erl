...
get_spec_field(DataElem)->
		case DataElem of
			...
			6 	->{n,12,fx,0,<<"Amount, cardholder billing">>};%%not in postillion
			7 	->{n,10,fx,0,<<"Transmission date & time MMDDhhmmss">>};
			8 	->{n,8,fx,0,<<"Amount, Cardholder billing fee">>};%%not in postillion
			9 	->{n,8,fx,0,<<"Conversion rate, Settlement">>};
			10	->{n,8,fx,0,<<"Conversion rate, cardholder billing">>};%%not in postillion
			11	->{n,6,fx,0,<<"System Trace Audit Number">>};
			12	->{n,6,fx,0,<<"Time, Local transaction (hhmmss)">>};%%different in postillion
			13	->{n,4,fx,0,<<"Date, Local transaction (MMDD)">>};
			14	->{n,4,fx,0,<<"Date, Expiration YYMM">>};
			15	->{n,4,fx,0,<<"Settlement Date MMDD">>};%%different in postillion
			16	->{n,4,fx,0,<<"Conversion Date MMDD">>};%%different in postillion
			17	->{n,4,fx,0,<<"Date, Capture MMDD">>};%%not in postillion
			18	->{n,4,fx,0,<<"Merchant Type">>};
			19	->{n,3,fx,0,<<"Country Code">>};%%not in postillion
