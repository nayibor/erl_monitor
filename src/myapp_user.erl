...
outa(Arg, 'GET', [_, "user", "get_users"]) ->
    ...
    {ok, UiData} = yapp_test_users_list:render([{title,
						 Title_Page},
						{yapp_prepath,
						 yapp:prepath(Arg)},
						{data, Users}]),
...
outa(Arg, 'GET', [_, "user", "search_user"]) ->
    ...
    case yaws_api:queryvar(Arg, "filter") of
	  ...
	  {ok, UiData} =
	      yapp_test_user_search:render([{yapp_prepath,
					     yapp:prepath(Arg)},
					    {data, Users}]),
	  {ehtml, UiData};
      undefined ->
...
outa(_Arg, 'GET', [_, "user", "get_add_user"]) ->
    ...
    {ok, UiData} = yapp_test_add_user:render([{sites_sys,
					       Sites},
					      {type_user_tran, "add_user"}]),
    {html, UiData};
...
outa(Arg, 'POST', [_, "user", "save_add_user"]) ->
    ...
    case yaws_api:postvar(Arg, "id") of
	  ...
	  case
	    yapp_test_lib_usermod:edit_user(list_to_integer(Id),
					    Email, Fname, Lname,
	  ...
	  {ok, Lname} = yaws_api:postvar(Arg, "lname"),
	  {ok, Siteid} = yaws_api:postvar(Arg, "site"),
	  case yapp_test_lib_usermod:add_user(Email, Fname, Lname,
					      list_to_integer(Siteid))
	      of
	    ok ->
		yapp_test_lib_util:message_client(200,
...
outa(Arg, 'POST', [_, "user", "save_roles_user"]) ->
    case yaws_api:postvar(Arg, "roles") =:= undefined orelse
	   yaws_api:postvar(Arg, "id") =:= {ok, []} orelse
	     yaws_api:postvar(Arg, "id") =:= undefined
	of
      true ->
	  yapp_test_lib_util:message_client(500,
	  ...
	  case yapp_test_lib_usermod:add_user_role_list(Id,
							Roles_save)
	      of
	    {error, Reason} ->
		yapp_test_lib_util:message_client(500,
...
outa(_Arg, 'POST',
     [_, "user", "reset_pass_user", UserId]) ->
    Result =
	yapp_test_lib_usermod:reset_pass(list_to_integer(UserId)),
    case Result of
      {ok, NewPass} ->
	  yapp_test_lib_util:message_client(200,
					    "New Pass is " ++ NewPass);
...
%%		return a messagepack object showing status
outa(_Arg, 'POST',
     [_, "user", "lock_account_user", UserId]) ->
    Result =
	yapp_test_lib_usermod:lock_account(list_to_integer(UserId)),
    case Result of
      ok ->
	  yapp_test_lib_util:message_client(200,
					    "Account Locked");
...
%%		return a messagepack object showing status
outa(_Arg, 'POST',
     [_, "user", "unlock_account_user", UserId]) ->
    Result =
	yapp_test_lib_usermod:unlock_account(list_to_integer(UserId)),
    case Result of
      ok ->
	  yapp_test_lib_util:message_client(200,
					    "Account Unlocked");
