...
outa(_Arg, 'GET', [_, "stats", "index_view_stats"]) ->
    ...
    Query =
	"SELECT task_name FROM realtime_data.dbo.tasks;",
    Param = [],
    case erlmon_worker_pool:query(Query, Param) of
      {ok, {_, _, Tasks}} ->
	  {ok, UiData} = yapp_test_content_insert:render([{title,
							   Title_Page},
							  {page_type,
							   "view_stats"},
							  {status, "ok"},
...
outa(Arg, 'GET', [_, "stats", "get_stats"]) ->
    ...
    case Start_date =:= undefined orelse
	   erlang:length(Start_date) > 23 orelse
	     erlang:length(End_date) > 23 orelse
	       End_date =:= undefined orelse Task_type =:= undefined
	of
      true ->
	  yapp_test_lib_util:message_client(500,
					    "Required Field is Empty Or Size Is Incorrect");
      _ ->
	  Data = lists:filtermap(fun (Task) ->
					 case erlang:length(Task) =< 30 of
					   true ->
					       {true,
						{{sql_varchar, 30}, [Task]}};
					   _ -> false
				 ...
				 string:tokens(Task_type, ",")),
	  Query_first =
	      "SELECT * FROM realtime_data.dbo.task_uptime "
	      " where date_begin >=? and date_begin "
	      "<= cast(? as datetime)+1 and task_name "
	      "in ",
	  Middle = ["(",
		    string:join(lists:duplicate(erlang:length(Data), "?"),
				","),
		    ")"],
	  Rest = " order by date_begin ASC;",
	  Query = lists:flatten([Query_first, Middle, Rest]),
	  %%io:format("~n query is ~p",[Query]),
	  Param = [{{sql_varchar, 23}, [Start_date]},
		   {{sql_varchar, 23}, [End_date]}
		   | Data],
	  case erlmon_worker_pool:query(Query, Param) of
	    {ok, {_, _, Tasks}} ->
		...
		{content, "application/octet-stream",
		 erlang:term_to_binary(Tasks)};
	    {error, _Reason} ->
		yapp_test_lib_util:message_client(500,
						  "There was an error getting the data.<b>Please "
						  "try again</b>")
	  end
    end;
