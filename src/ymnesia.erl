...
- define ( elog ( X , Y ) , error_logger : info_msg ( "*elog ~p:~p: " X , [ ? MODULE , ? LINE | Y ] ) ) .


%%% Possible to call mnesia in another way
-define(MNESIA(Mod, Args),
	apply(mnesia, Mod,
	      Args)).%%-define(MNESIA(Mod, Args), fdapi:call_in_master(mnesia, Mod, Args)).

%%% @private
out(A) ->
    %%?elog("Inside Ymnesia~n", []),
    %io:format("~nappmod data ~p",[A#arg.appmoddata]),
    Uri = yaws_api:request_url(A),
    case string:tokens(Uri#url.path, "/") of
      [_, "ymnesia", "table"] ->
	  %%["table" | _] ->
	  case is_post(A) of
	    true ->
	    ...
	    false -> return_top_page()
	  end;
      _ -> return_top_page()
    end.

%%% Get the fields to be part of the select
select_fields([{"tablename", _} | T]) ->
    select_fields(T);
...
select_pattern(Name, Ls) ->
    ...
    mk_select_pattern([select_pattern_1(V1)
		       || V1 <- get_attributes(l2a(Name))],
		      Ls, Wp, 2).

select_pattern_1(A) -> a2l(A).

mk_select_pattern([A | As], [{A, V} | T], Wp, N) ->
    mk_select_pattern(As, T, setelement(N, Wp, be_smart(V)),
		      N + 1);
mk_select_pattern([_ | As], L, Wp, N) ->
    mk_select_pattern(As, L, Wp, N + 1);
mk_select_pattern([], [], Wp, _) -> Wp.
...
be_smart([H | T], integer) when H > $. ->
    be_smart(T, float);
be_smart([_ | T], _) -> be_smart(T, list);
be_smart([], Type) -> Type.
...
style() ->
	...
	["table {border-collapse: collapse; border: "
	 ...
	 "attribute {vertical-align: top; font-size: "
	 "80%;}\nth {padding: 5px; border: solid "
	 "black 1px;}\ntd {padding: 5px; border: "
	 "solid black 1px;}\n"]}]}].
...
table(Cbox, Sp, Table) when is_atom(Table) ->
    case catch (?MNESIA(table_info, [Table, attributes])) of
	  ...
	  Vp = view_pattern(Cbox, [table_1(V1) || V1 <- Headers]),
	  {Q, Result} = do_query(Sp),
	  {ehtml,
	   [{head, [], [meta() ++ style()]},
	    {body, [],
	     [{'div', [], {p, [], "Query: " ++ Q}},
	      {'div', [], {p, [], "Table: " ++ a2l(Table)}}
...
view_pattern([Cbox | Cs], [Cbox | T], N) ->
    [N | view_pattern(Cs, T, N + 1)];
view_pattern(Cs, [_ | T], N) ->
    view_pattern(Cs, T, N + 1);
view_pattern([], [], _) -> [].
...
mk_table_tab() ->
    Rows = get_tables(),
    [{'div', [],
      [{table, [], [mk_table_tab_1(V1) || V1 <- Rows]}]}].

mk_table_tab_1(Row) ->
    {tr, [],
     {form,
      [{action, "ymnesia/table"}, {method, "post"},
       {name, Row}],
      [{td, [], sublnk(a2l(Row))} | mk_input_fields(Row)]}}.

%%% Create each table cell; consisting of the attribute name and an input field.
mk_input_fields(Table) ->
    As = get_attributes(Table),
    Max = max_noof_attrs(),
    [mk_input_fields_1(V1)
     || V1 <- As ++ lists:duplicate(Max - length(As), 0)].

mk_input_fields_1(0) -> {td, [], []};
mk_input_fields_1(Attribute) ->
    A = a2l(Attribute),
    {td, [],
...
extract_cbox(L) -> extract_cbox(L, [], []).

extract_cbox([{"cbox_" ++ Cbox, _} | T], Cs, Rs) ->
    extract_cbox(T, [Cbox | Cs], Rs);
extract_cbox([H | T], Cs, Rs) ->
    extract_cbox(T, Cs, [H | Rs]);
extract_cbox([], Cs, Rs) -> {reverse(Cs), reverse(Rs)}.

%%% Build the result table.
mk_tab(Vp, Headers, Rows) ->
    [{'div', [],
      [{table, [],
	[{tr, [], [{th, [], a2l(X)} || X <- vp(Vp, Headers)]}
	 | [mk_tab_1(V1, Vp) || V1 <- Rows]]}]}].

mk_tab_1(Row, Vp) ->
    {tr, [], [{td, [], massage(W)} || W <- vp(Vp, Row)]}.
...
sublnk(E) ->
    {a, [{href, "#"}, {onclick, E ++ ".submit();"}],
     [E,
      {input,
       [{type, "hidden"}, {name, "tablename"}, {value, E}]}]}.

massage(W) -> lists:flatten(io_lib:format("~p", [W])).

do_query(Sp) ->
    {lists:flatten(io_lib:format("mnesia:match_object(~p)",
				 [Sp])),
     lists:keysort(2, ?MNESIA(dirty_match_object, [Sp]))}.

error_page(Msg) -> {html, [Msg]}.
...
lk(Key, L) ->
    {value, {_, Val}} = lists:keysearch(Key, 1, L), Val.

t2l(L) -> [t2l_1(V1) || V1 <- L].

t2l_1(T) -> tail(tuple_to_list(T)).

tail([]) -> [];
tail(L) -> tl(L).

str2term(Str) -> hd(str2terms(Str ++ ". ")).
...
tokenlists2terms(Lists) ->
    [tokenlists2terms_1(V1) || V1 <- Lists].

tokenlists2terms_1(L) ->
    {ok, Term} = erl_parse:parse_term(L), Term.
