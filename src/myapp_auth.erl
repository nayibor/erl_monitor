...
-export([crashmsg/3, out/1, out/2, out/3, out401/3,
	 out404/3]).
...
out(Arg, error) ->
    ...
    case Username =:= undefined orelse
	   Password =:= undefined orelse
	     Username =:= "" orelse Password =:= ""
	of
      true -> {page, yapp:prepath(Arg) ++ (?INDEX_PAGE)};
      _ ->
	  case yapp_test_lib_usermod:verify_user(Username,
						 Password)
	      of
	    {ok, Id, Fname, Site_id, Inst_id, false} ->
		User_Links = yapp_test_lib_usermod:get_user_links(Id),
		Cookie = yapp_test_lib_sess:setup_session(Id, Fname,
							  Site_id, Inst_id,
							  ?SESSION_MAX_TIME,
							  User_Links),
		Data = {redirect_local,
			yapp:prepath(Arg) ++ (?PG_DASHBOARD)},
		CO = yaws_api:set_cookie(?COOKIE_VARIABLE, Cookie,
					 [{path, yapp:prepath(Arg)}]),
		[Data, CO];
		...
		Cookie = yapp_test_lib_sess:setup_session(Id, Fname,
							  ...
							  []),
		Data = {redirect_local,
			yapp:prepath(Arg) ++ (?PG_RESET_PASS)},
		CO = yaws_api:set_cookie(?COOKIE_VARIABLE, Cookie,
					 [{path, yapp:prepath(Arg)}]),
		[Data, CO];
	    _ -> {page, yapp:prepath(Arg) ++ (?INDEX_PAGE)}
	  end
    end;
...
out(Arg, ok) ->
    {redirect_local,
     yapp:prepath(Arg) ++
       (?PG_DASHBOARD)}.                %%{page,yapp:prepath(Arg)++?PG_DASHBOARD}.
...
crashmsg(_Arg, _SC, Str) ->
    error_logger:error_msg("~nError is ~p ~ nStacktrace is ~p",
			   [Str, erlang:get_stacktrace()]),
    {html,
     "Crashed Page"}.        %%  {html,"<h1>pg unauth</h1>"}.
