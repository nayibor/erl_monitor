...
outa(_Arg, 'GET',
     [_, "dashboard", "index_dashboard_view"]) ->
...
outa(Arg, 'GET', [_, "dashboard", "index_dashboard"]) ->
    Title_Page = "Welcome to Monitor",
    [{_, Name}, {_, Links_Allowd}] =
	yapp_test_lib_sess:get_user_data(Arg, ?COOKIE_VARIABLE),
    {ok, UiData} = yapp_sidebar:render([{fname, Name},
					{title, Title_Page},
