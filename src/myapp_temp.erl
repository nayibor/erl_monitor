...
outa(Arg, 'POST', [_, "temp", "save_add_temp"]) ->
    case yaws_api:postvar(Arg, "ident") =:= undefined orelse
	   yaws_api:postvar(Arg, "description") =:= undefined
		 ...
		 yaws_api:postvar(Arg, "category") =:= {ok, ""}
	of
      true ->
	  yapp_test_lib_util:message_client(500,
	  ...
	  case yaws_api:postvar(Arg, "id") of
		...
		case
		  yapp_test_lib_isoproc:edit_template(list_to_integer(Edit_id_val),
		...
		{ok, Cat} = yaws_api:postvar(Arg, "category"),
		case
		  yapp_test_lib_isoproc:add_template(list_to_binary(Ident),
						     list_to_binary(Description),
						     list_to_integer(Cat))
		    of
		  ok ->
...
outa(_Arg, 'DELETE', [_, "temp", "del_temp", TempId]) ->
    ...
    yapp_test_lib_util:message_client(200,
				      "Template And Template Rules Deleted");
