...
outa(Arg, 'POST', [_, "rules", "save_add_rule"]) ->
    ...
    case yaws_api:postvar(Arg, "siteid") =:= undefined
	   orelse
	   yaws_api:postvar(Arg, "templateid") =:= undefined orelse
	     yaws_api:postvar(Arg, "description") =:= undefined
	       orelse
	...
	of
	  ...
	  case yaws_api:postvar(Arg, "id") of
		...
		{ok, Description} = yaws_api:postvar(Arg,
						     "description"),
...
outa(Arg, 'POST', [_, "rules", "save_rule_members"]) ->
    ...
    case yapp_test_lib_rules:save_member_rules(Id,
					       Rules_save)
	of
      ok ->
	  yapp_test_lib_util:message_client(200,
