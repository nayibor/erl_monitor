...
out(Arg) ->
    out(Arg,
	yapp_test_lib_sess:check_login(Arg,
				       ?COOKIE_VARIABLE)).        %%io:format("~n~pcookie::",[Arg]),
...
outa(Arg, 'GET', [_, "websock", "setup"]) ->
    Data_user = yapp_test_lib_sess:get_user_websocket(Arg,
						      ?COOKIE_VARIABLE),
...
outa(Arg, 'GET', [_, "websock", "mithril"]) ->
    {ok, UiData} =
	yapp_test_mithril_layout:render([{yapp_prepath,
					  yapp:prepath(Arg)}]),
    {html, UiData};
