...
install(Nodes)->
		...
		mnesia:create_table(usermod_categories,
							[{attributes, record_info(fields, usermod_categories)},
							 ...
							 {disc_copies, Nodes}]),								 				 	 
		lists:map(fun(X)->add_auto_table(X) end,[usermod_inst,usermod_categories,usermod_sites,usermod_users,usermod_roles,usermod_links,usermod_users_roles,usermod_role_links]),					 							                                            
